...
exports.list = async (req, res) => {
    try {
        ...
        let sql = `
      SELECT b.booking_id, b.user_id, b.showtime_id, b.seats, b.status, b.booked_at,
...
// POST /bookings  { user_id, showtime_id, seats }
exports.create = async (req, res) => {
    try {
        let { user_id, showtime_id, seats } = req.body;
        ...
        try {
            ...
            // สร้าง booking (เริ่มที่ pending)
            const [result] = await conn.execute(
                'INSERT INTO bookings (user_id, showtime_id, seats, status) VALUES (?, ?, ?, ?)',
                [user_id, showtime_id, seats, 'pending']
            );
            ...
            res.status(201).json({
                ...
                booking: { booking_id: result.insertId, user_id, showtime_id, seats, status: 'pending' }
            });
        } catch (e) {
            await db.query('ROLLBACK'); // บางกรณีถ้า conn เสีย ให้พยายาม rollback
...
// DELETE /bookings/:id   (ถ้ายังไม่ cancelled จะคืนที่นั่งก่อนลบ)
exports.remove = async (req, res) => {
    ...
    try {
        ...
        // ถ้ายังไม่คืนที่นั่ง (pending/confirmed) ให้คืนก่อน
        if (status !== 'cancelled') {
            await conn.execute(
                'UPDATE showtimes SET available_seats = available_seats + ? WHERE showtime_id = ?',
                [seats, showtime_id]
...
exports.historyByUser = async (req, res) => {
    ...
    `;
        ...
        if (status) { sql += ' AND b.status = ?'; params.push(status); }
        if (from) { sql += ' AND b.booked_at >= ?'; params.push(from); }   // YYYY-MM-DD หรือ datetime
...
// GET /me/bookings?status=&from=&to=&page=&limit=   (ต้องมี auth เติม req.user.id)
exports.myHistory = async (req, res) => {
    // ต้องมี middleware auth ที่เซ็ต req.user.id จาก JWT
